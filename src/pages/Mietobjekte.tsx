...
export default function Mietobjekte() {
  ...
  const mietProperties = content.properties.filter((p) => p.type === 'mieten');
  ...
  return (
    <>
      ...
      <section className="relative h-[40vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        <EditableImage
          src={content.mietHeroImage}
          onSave={(url) => handleSave('mietHeroImage', url)}
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          ...
          >
            <EditableText
              value={content.mietTitle}
              onSave={(v) => handleSave('mietTitle', v)}
              as="h1"
              className="text-4xl md:text-5xl lg:text-6xl font-display font-bold text-white mb-4"
            />
            <EditableText
              value={content.mietSubtitle}
              onSave={(v) => handleSave('mietSubtitle', v)}
              as="p"
      ...
      <section className="py-24 bg-brand-light min-h-[60vh] -mt-8 rounded-t-[2.5rem] relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {mietProperties.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {mietProperties.map((property, index) => (
                <motion.div
                  key={property.id}
                  initial={{ opacity: 0, y: 30 }}
            ...
            >
              ...
              <p className="text-brand-gray max-w-md mx-auto mb-8">
                Derzeit sind keine Mietobjekte verfügbar. Bitte schauen Sie später wieder vorbei oder kontaktieren Sie uns direkt.
              </p>
              <Link
                to="/kontakt"
